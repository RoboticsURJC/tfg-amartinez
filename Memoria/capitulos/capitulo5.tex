...
\begin{itemize}
    ...
    \item \textbf{Módulo de actuación:} encargado de la dispensación física de la medicación. A partir de las órdenes generadas por el módulo de control, acciona un servomotor acoplado a un mecanismo de trinquete con retorno mediante muelle.\\

    El conjunto incorpora además un sistema de bloqueo que evita movimientos no deseados de la cama que porta las tomas de pastillas. Gracias a esta disposición, el sistema realiza avances discretos y controlados.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figs/mod_coms_diagram.jpg}
...
Durante el desarrollo del sistema se han tenido en cuenta diversos criterios de diseño con el objetivo de garantizar su funcionalidad, accesibilidad y fiabilidad en un entorno doméstico. Información adicional sobre el diseño mecánico y electrónico puede consultarse en la Wiki del proyecto.\footnote{\url{https://github.com/RoboticsURJC/tfg-amartinez/wiki/3D-Design}, \url{https://github.com/RoboticsURJC/tfg-amartinez/wiki/Electronic-Design}}\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figs/esquema.jpg}
...
En el diseño del \textbf{sistema de dispensación} se evaluaron distintas soluciones mecánicas para garantizar un posicionamiento preciso de los compartimentos del pastillero. Entre las alternativas consideradas se encontraba un mecanismo basado en una rueda de Ginebra, ampliamente utilizado en aplicaciones que requieren movimientos intermitentes y posicionamientos discretos. Esta solución permitía obtener una elevada precisión angular y una separación clara entre posiciones consecutivas.\\

No obstante, tras analizar los requisitos del proyecto y las características del prototipo desarrollado, se optó finalmente por un mecanismo de trinquete accionado mediante servomotor. El diseño incorpora un sistema de empuje, un muelle de retorno y un elemento de bloqueo que permite realizar avances controlados del carrusel, asegurando que cada compartimento quede correctamente posicionado tras cada actuación. Esta solución presenta una menor complejidad mecánica, resulta más sencilla de fabricar mediante impresión 3D y ofrece una fiabilidad adecuada para las necesidades del sistema.
...
La interfaz del dispositivo se ha desarrollado utilizando la librería LVGL, una herramienta ampliamente utilizada en sistemas embebidos para la creación de interfaces gráficas eficientes. Permite implementar una interfaz táctil fluida, optimizada para dispositivos con recursos limitados, como el microcontrolador utilizado en el sistema. El desarrollo de la interfaz y la configuración de LVGL se documentan en la Wiki del proyecto\footnote{
\url{https://tinyurl.com/4vku9wkv},
\url{https://tinyurl.com/2e3ucavb},
\url{https://tinyurl.com/mr29rfsp}}.\\
...
\begin{itemize}
    ...
    Para simplificar este proceso, el dispositivo genera dinámicamente un código QR que contiene la información necesaria para la configuración. Este enfoque evita la introducción manual de datos y reduce la posibilidad de errores por parte del usuario.\\

    El código QR se construye a partir de una cadena de texto y se transforma en una representación gráfica que es mostrada en pantalla mediante la librería LVGL. En el Código~\ref{cod:qr} se muestra un fragmento simplificado del proceso de generación y visualización del QR:
...
La aplicación móvil constituye la interfaz remota del sistema, permitiendo al usuario configurar el dispositivo y acceder a sus funcionalidades de forma inalámbrica. Utilizando las herramientas explicadas en el capítulo anterior, se ha desarrollado una aplicación que proporciona una interfaz sencilla e intuitiva, centrada en la gestión de tomas, la configuración del dispositivo y la visualización de datos del sensor biomédico. Su evolución se encuentra documentada en la Wiki del proyecto\footnote{\url{https://github.com/RoboticsURJC/tfg-amartinez/wiki/App-Development}}.\\
...
\begin{itemize}
    ...
    \item Vinculación segura con el dispositivo mediante token.
    \item Autenticación mediante PIN.
...
Por motivos de seguridad, el acceso a la aplicación se encuentra protegido mediante un PIN configurable por el usuario. Este código debe introducirse al iniciar la aplicación y permite restringir el acceso a la información almacenada y a las opciones de configuración del sistema. El PIN se mantiene sincronizado con el dispositivo, garantizando que ambas plataformas compartan la misma configuración de autenticación.\\

El funcionamiento de la aplicación sigue un flujo estructurado que comienza con la configuración inicial del dispositivo. En primer lugar, el usuario accede al apartado de configuración, donde se muestra un lector de códigos QR que permite escanear el código generado por el dispositivo. El código contiene un token único de vinculación que permite identificar el dispensador y asociarlo de forma segura con la aplicación móvil.\\
...
No es necesario repetir la configuración en usos posteriores. La aplicación conserva la información necesaria para identificar el dispositivo y es capaz de localizarlo automáticamente cuando ambos se encuentran conectados a la misma red. De este modo, el usuario puede acceder directamente a las funcionalidades del sistema sin necesidad de realizar nuevamente el proceso de emparejamiento. Únicamente será necesario repetir la vinculación en situaciones excepcionales, como la eliminación de los datos almacenados en la aplicación o el restablecimiento de la configuración del dispositivo.\\

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figs/flujo_app.jpg}
    \caption{Esquema que representa el flujo de uso de la app (imagen generada por IA)}
    \label{fig:app_movil}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{figs/collage_app.jpg}
...
Esta aproximación permite desacoplar la configuración de las tomas de la lógica de ejecución del sistema, facilitando su modificación desde la interfaz de usuario y su sincronización con la aplicación móvil. Para garantizar la persistencia de la configuración, el sistema utiliza \textit{Non-Volatile Storage} (NVS), implementada mediante la librería \texttt{Preferences.h} del entorno de desarrollo de ESP32, lo que permite conservar los datos incluso tras reiniciar el dispositivo.\\

La información de cada toma se serializa y almacena mediante claves específicas. Para optimizar el almacenamiento, los días de repetición se codifican en un único byte utilizando una representación binaria, donde cada bit corresponde a un día de la semana. Un ejemplo simplificado del proceso de almacenamiento se muestra en el Código~\ref{cod:nvs}:
...
La comunicación entre el dispositivo y la aplicación móvil se realiza mediante conexión WiFi, permitiendo tanto la configuración inicial del sistema como el intercambio de información durante su funcionamiento\footnote{\url{https://tinyurl.com/mryfxb8a}}.\\
...
Para facilitar la conexión entre la aplicación móvil y el dispositivo, se ha implementado un mecanismo de descubrimiento que permite localizar automáticamente la ESP32 dentro de la red local. Este proceso evita que el usuario tenga que introducir manualmente la dirección IP del dispositivo, mejorando la experiencia de uso y simplificando la interacción con el sistema.

En el funcionamiento normal, la aplicación intenta inicialmente establecer comunicación utilizando la última dirección IP almacenada del dispositivo. Si la comprobación falla, se inicia un proceso de descubrimiento automático mediante difusión UDP (\textit{broadcast}) en la red local. Para ello, la aplicación envía un mensaje de descubrimiento al puerto correspondiente y espera la respuesta de la ESP32, obteniendo automáticamente su dirección IP.

\begin{lstlisting}[language=Kotlin, caption={Descubrimiento automático del dispositivo mediante difusión UDP}, label={cod:discover_esp}]
private fun discoverEsp(): String? {
    ...
    return if (
        String(response.data, 0, response.length) == "ESP_HERE"
    ) {
...
Una vez obtenida la dirección IP, la aplicación verifica la conectividad mediante el endpoint \texttt{/ping}. Si la comprobación es satisfactoria, la dirección se almacena para futuras conexiones y, si es necesario, se solicita automáticamente un nuevo token mediante el endpoint \texttt{/link}, permitiendo establecer la comunicación de forma transparente y adaptándose a posibles cambios en la dirección IP del dispositivo.
...
El sistema de dispensación constituye el núcleo funcional del dispositivo, siendo el encargado de liberar la medicación de forma controlada en los instantes programados por el sistema. Para ello, se ha desarrollado un mecanismo capaz de posicionar secuencialmente los compartimentos del carrusel y garantizar que la dosis correspondiente quede alineada con la zona de dispensación.\\

El accionamiento del mecanismo se realiza mediante un servomotor SG90 controlado por la unidad central del sistema. La elección de este actuador responde a su bajo coste, reducido consumo energético y facilidad de integración con la plataforma hardware empleada.

\begin{figure}[H]
\centering
\includegraphics[width=0.65\textwidth]{figs/mecanismo.jpeg}
\caption{Sistema de dispensación basado en mecanismo de trinquete}
\label{fig}
\end{figure}

La Figura~\ref{fig} muestra el mecanismo de dispensación desarrollado y su integración dentro de la estructura del carrusel.

El mecanismo implementado está compuesto por un elemento impulsor accionado por el servomotor, un sistema de trinquete encargado de producir el avance del carrusel y un mecanismo de bloqueo asistido por muelle que impide movimientos no deseados entre posiciones consecutivas. Gracias a esta configuración, cada actuación del servomotor provoca un avance controlado, mientras que el sistema de retención garantiza la estabilidad de la posición alcanzada.\\

Esta solución permite obtener un sistema de dispensación fiable utilizando componentes de bajo coste y elementos fabricados mediante impresión 3D, proporcionando un posicionamiento repetible de los compartimentos y una correcta dispensación de la medicación programada.

\subsection{Sistema biomédico}

Con el objetivo de complementar las funcionalidades del dispensador, se ha incorporado un sistema de monitorización biomédica basado en el sensor MAX30102. Este sensor integra emisores LED y un fotodiodo capaces de realizar mediciones ópticas mediante fotopletismografía (PPG), técnica ampliamente utilizada para estimar parámetros fisiológicos como la frecuencia cardíaca. La Figura~\ref{fig:max30102_flow} resume las diferentes etapas empleadas para obtener la frecuencia cardíaca a partir de la señal capturada por el sensor.\\

La obtención de la frecuencia cardíaca se basa en el análisis de la señal infrarroja capturada por el sensor. Para ello, el sistema realiza una lectura continua de la intensidad reflejada y emplea un algoritmo de detección de pulsos que identifica los máximos asociados a cada latido cardíaco.\\

Una vez detectado un latido, se calcula el tiempo transcurrido desde el anterior y, a partir de este intervalo, se obtiene una estimación instantánea de la frecuencia cardíaca en pulsaciones por minuto (BPM).\\

\begin{lstlisting}[language=C, caption={Cálculo instantáneo de la frecuencia cardíaca}]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/sensor_esquema.jpg}
    \caption{Proceso de obtención de la frecuencia cardíaca mediante el sensor MAX30102 (imagen generada por IA)}
    \label{fig:max30102_flow}
\end{figure}

Debido a la presencia de ruido y pequeñas variaciones producidas por el movimiento del usuario o por las condiciones de adquisición de la señal, las mediciones instantáneas pueden presentar oscilaciones significativas. Para mejorar la estabilidad de los resultados, el sistema almacena las últimas medidas válidas en un buffer circular de diez muestras.\\

Posteriormente, se calcula el valor medio de dichas muestras y se aplica un filtro exponencial que suaviza las variaciones entre medidas consecutivas.\\

\begin{lstlisting}[language=C, caption={Filtrado de las mediciones obtenidas}]
filteredBpm =
0.9f * filteredBpm +
0.1f * beatAvg;
\end{lstlisting}
...
Este procedimiento permite reducir la influencia del ruido presente en la señal y obtener una estimación más estable y representativa de la frecuencia cardíaca del usuario.

\subsection{Integración electrónica}
\label{sec:integracion_electronica}

El sistema electrónico se basa en un microcontrolador ESP32, encargado de la gestión del conjunto del dispositivo.\\

Los distintos elementos hardware se conectan al microcontrolador mediante interfaces específicas. El sensor biomédico se integra a través del protocolo I2C, lo que simplifica el cableado del sistema. Asimismo, el sistema se alimenta mediante una conexión USB-C, proporcionando una solución estándar y ampliamente compatible para su uso en entornos domésticos.\\

Para facilitar el montaje del dispositivo, se ha diseñado una placa de circuito impreso (PCB) que organiza las conexiones entre los distintos componentes.De esta forma, el sistema es más estable y se reduce la complejidad del cableado respecto a soluciones basadas únicamente en prototipado.\\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/montaje.jpg}
